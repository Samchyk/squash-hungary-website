    ...
    <main className="min-h-screen flex flex-col">
      ...
      <section className="py-0 md:py-8 px-4 md:px-8 max-w-7xl mx-auto w-full mb-12">
        <div className="relative w-full h-96 md:h-[500px] overflow-hidden rounded-sm animate-fade-in">
          <Image
            src="/squash-history.jpg"
            alt="Squash sport history timeline"
            fill
      ...
      <section className="py-20 md:py-32 px-4 md:px-8 max-w-7xl mx-auto">
        <div className="space-y-12 md:space-y-16">
          {timeline.map((event, idx) => (
            <div 
              key={idx} 
              className="flex gap-8 md:gap-12 animate-slide-in-left"
              style={{
                animationDelay: `${idx * 150}ms`,
              }}
            >
              <div className="flex flex-col items-center flex-shrink-0">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-accent text-accent-foreground rounded-full flex items-center justify-center font-playfair font-bold text-lg md:text-xl hover:scale-110 transition-transform duration-300 shadow-lg">
                  {idx + 1}
                </div>
                {idx < timeline.length - 1 && (
                  <div className="w-1 h-24 md:h-32 bg-border mt-4 animate-pulse-subtle" />
                )}
              </div>
              <div className="pt-2 pb-8 md:pb-12 hover:translate-x-2 transition-transform duration-300">
                <p className="text-accent font-bold text-sm uppercase tracking-wide">{event.year}</p>
                <h3 className="font-playfair text-2xl md:text-3xl font-bold mt-2 mb-3 hover:text-accent transition-colors duration-300">
                  {event.title}
                </h3>
      ...
      <section className="py-16 md:py-24 px-4 md:px-8 bg-primary text-primary-foreground">
        <div className="max-w-7xl mx-auto">
          <h2 className="font-playfair text-4xl font-bold mb-12 animate-fade-in">Érdekes Tények</h2>
          
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-4 p-8 rounded-sm bg-primary-foreground/5 hover:bg-primary-foreground/10 transition-all duration-300 animate-scale-in">
            ...
            <div className="space-y-4 p-8 rounded-sm bg-primary-foreground/5 hover:bg-primary-foreground/10 transition-all duration-300 animate-scale-in" style={{ animationDelay: '100ms' }}>
